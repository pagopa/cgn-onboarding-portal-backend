...
public enum DocumentTypeEnum {
    ...
    BACKOFFICE_ADHESION_REQUEST("BACKOFFICE_ADHESION_REQUEST", Type.ADHESION_REQUEST, true);
    ...
    public static int getNumberOfDocumentProfile() {
        return (int) Arrays.stream(DocumentTypeEnum.values()).filter(d -> !d.isBackoffice()).count();
    }

    public static DocumentTypeEnum fromValue(String value) {
        return Arrays.stream(DocumentTypeEnum.values())
                     .filter(typeEnum -> typeEnum.getCode().equalsIgnoreCase(value))
                     .findFirst()
