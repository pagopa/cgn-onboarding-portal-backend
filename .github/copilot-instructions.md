...
- `/agreement-requests` is the pre-approval backoffice surface and can include `DRAFT` and `PENDING`; `AssignedAgreement` remains a DTO-only projection of `PENDING` plus `backofficeAssignee`. `/approved-agreements` is the broader post-approval backoffice surface and can include `APPROVED`, `ACTIVE`, `INACTIVE`, `TERMINATION_IN_PROGRESS`, and `TERMINATED`.
